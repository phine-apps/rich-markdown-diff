...
- [ ] **Linear Parsing Fallback**: For nested or paired block structures, use deterministic depth-tracking functions (e.g., [`findClosing`](file:///Users/oishik/apps/rich-markdown-diff/src/markdown/domUtils.ts)) instead of monolithic regular expressions.
...
- [ ] **Iterative Tag Stripping**: Never use single-pass `replace(/<[^>]+>/g, "")` for sanitization. Always use [`stripHtmlTags()`](file:///Users/oishik/apps/rich-markdown-diff/src/markdown/domUtils.ts) with `do ... while` loops.
- [ ] **Sanitizer Configuration**: Ensure any rendered HTML passed to Webviews passes through [`sanitizeHtml()`](file:///Users/oishik/apps/rich-markdown-diff/src/markdown/sanitizer.ts) with strict tag and attribute whitelists.
